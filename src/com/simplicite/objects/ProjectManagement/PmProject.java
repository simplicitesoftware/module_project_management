...
public class PmProject extends ObjectDB {
	...
	public String pubHtml(){
		...
		String html="<head><title>"+getFieldValue("pmPrjName")+": "+now+"</title></head>"+"\n";
		...
		for(String[] row : getGrant().query(sqlQuery)){
			...
			String sqlQueryTask = "select row_id, pm_tsk_number, pm_tsk_title,pm_tsk_description,pm_tsk_status,pm_tsk_priority,pm_tsk_effective_closing_date,pm_tsk_expected_duration  from pm_task where pm_tsk_vrs_id="+row[0]+"order by pm_tsk_creation DESC";
			int id = 0; int number = 1; int title= 2; int description= 3; int status= 4; int priority= 5; int effectiveClosingDate= 6; int expectedDuration=7;
			html += "<table>"+"\n";
			html += "<thead><tr><th>Number</th><th>Title</th><th>Description</th><th>Status</th><th>Priority</th><th>Effective closing date</th><th>Expeted Duration</th></tr></thead>"+"\n";
			html += "<tbody>"+"\n";
